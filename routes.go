...
func buildRoutes(db *gorm.DB) *gin.Engine {
	...
	tableContrller := controllers.NewTableController(db)

	router.GET("/", tableContrller.CreateTable)

	// Users
	userControllerAll := controllers.NewUsersController(db)

	router.POST("/", userControllerAll.CreateUser)
